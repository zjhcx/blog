...
const DEFAULT_COVER = SITE_INFO.HomeBanner.background.match(/url\(['"]?([^'")]+)['"]?\)/)?.[1] || '/assets/images/lazy-loading.webp';
...
async function* createImageIterator(dir: string) {
  ...
  while (true) {
    ...
    for (const image of images) {
      yield `/assets/images/banner/${image}`;
    }
  }
}
