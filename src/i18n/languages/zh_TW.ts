import Key from "../i18nKey";
import type { Translation } from "../translation";

export const zh_TW: Translation = {
	[Key.home]: "首頁",
	[Key.about]: "關於",
	[Key.archive]: "彙整",
	[Key.links]: "友鏈",
	[Key.linksDescription]: "朋友們的部落格和推薦站點",
	[Key.other]: "其他",
	[Key.otherDescription]: "更多頁面與站點功能",
	[Key.search]: "搜尋",
	[Key.searchPanel]: "搜尋面板",
	[Key.searchDevTitle]: "這是一條假的搜尋結果",
	[Key.searchDevDescription]: "因為搜尋無法在 <mark>開發</mark> 環境中運作。",
	[Key.searchDevTestTitle]: "如果你想測試搜尋",
	[Key.searchDevTestDescription]:
		"請改為執行 <mark>npm build && npm preview</mark>。",
	[Key.language]: "語言",

	[Key.bangumi]: "追番",
	[Key.bangumiDrama]: "追劇",
	[Key.bangumiDescription]: "來自嗶哩嗶哩的番劇收藏",
	[Key.bangumiDramaDescription]: "來自嗶哩嗶哩的劇集收藏",
	[Key.bangumiDisabled]: "追番頁已關閉。",
	[Key.bangumiLoadFailed]: "載入嗶哩嗶哩追番資料失敗：",
	[Key.bangumiEmpty]: "暫無追番資料。",
	[Key.bangumiFollow]: "追番",
	[Key.bangumiPlay]: "播放",
	[Key.bangumiOtherDescription]: "查看 Bilibili 追番列表",
	[Key.bangumiDramaOtherDescription]: "查看 Bilibili 追劇列表",
	[Key.music]: "音樂",
	[Key.musicDescription]: "基於本地 JSON 或 API 資料驅動的播放器頁面",
	[Key.musicDisabled]: "音樂頁已關閉。",
	[Key.musicLoadFailed]: "載入音樂資料失敗：",
	[Key.musicEmpty]: "暫無音樂資料。",
	[Key.musicOtherDescription]: "打開站點內建音樂播放器",
	[Key.musicPlaylist]: "播放清單",
	[Key.musicPlaylistSearch]: "搜尋歌曲、歌手或專輯",
	[Key.musicNowPlaying]: "正在播放",
	[Key.musicUnknownArtist]: "未知歌手",
	[Key.musicUnknownAlbum]: "未知專輯",
	[Key.musicUploader]: "投稿者",
	[Key.musicArtistHome]: "歌手主頁",
	[Key.musicVideoLink]: "打開影片",
	[Key.musicPlay]: "播放",
	[Key.musicPause]: "暫停",
	[Key.musicPrevious]: "上一首",
	[Key.musicNext]: "下一首",
	[Key.musicPlayMode]: "播放模式",
	[Key.musicModeOrder]: "順序",
	[Key.musicModeLoop]: "單曲循環",
	[Key.musicModeRandom]: "隨機",
	[Key.musicVolume]: "音量",
	[Key.musicSpeed]: "倍速",
	[Key.musicLyrics]: "歌詞",
	[Key.musicLyricsLoading]: "歌詞載入中...",
	[Key.musicLyricsUnavailable]: "目前歌曲暫無歌詞。",
	[Key.musicNoMatch]: "沒有符合的歌曲。",
	[Key.musicTrackCount]: "首歌曲",

	[Key.moments]: "朋友圈",
	[Key.momentsDescription]: "來自 RSS 源的朋友最新動態",
	[Key.momentsDisabled]: "朋友圈已關閉。",
	[Key.momentsNoSources]:
		"暫無 RSS 源，請在 src/config.ts 的 MomentsConfig.sources 中配置。",
	[Key.momentsLoadFailed]: "載入失敗：",
	[Key.momentsEmpty]: "暫無朋友圈動態。",
	[Key.momentsUnknownTime]: "未知時間",
	[Key.momentsUntitled]: "未命名動態",
	[Key.momentsRequestFailed]: "RSS 請求失敗",
	[Key.momentsOtherDescription]: "查看 RSS 源聚合的朋友最新動態",

	[Key.follows]: "B站關注",
	[Key.followsDescription]: "來自嗶哩嗶哩的關注列表",
	[Key.followsDisabled]: "B站關注頁已關閉。",
	[Key.followsLoadFailed]: "載入嗶哩嗶哩關注資料失敗：",
	[Key.followsEmpty]: "暫無關注資料。",
	[Key.followsOfficial]: "認證",
	[Key.followsVip]: "大會員",
	[Key.followsOtherDescription]: "查看 Bilibili 關注列表",

	[Key.fans]: "B站粉絲",
	[Key.fansDescription]: "來自嗶哩嗶哩的粉絲列表",
	[Key.fansDisabled]: "B站粉絲頁已關閉。",
	[Key.fansLoadFailed]: "載入嗶哩嗶哩粉絲資料失敗：",
	[Key.fansEmpty]: "暫無粉絲資料。",
	[Key.fansOtherDescription]: "查看 Bilibili 粉絲列表",

	[Key.dynamic]: "B站動態",
	[Key.dynamicDescription]: "來自嗶哩嗶哩的空間動態",
	[Key.dynamicDisabled]: "B站動態頁已關閉。",
	[Key.dynamicLoadFailed]: "載入嗶哩嗶哩動態資料失敗：",
	[Key.dynamicEmpty]: "暫無動態資料。",
	[Key.dynamicOtherDescription]: "查看 Bilibili 空間動態",
	[Key.dynamicStatForward]: "轉發",
	[Key.dynamicStatComment]: "評論",
	[Key.dynamicStatLike]: "按讚",

	[Key.favs]: "B站收藏夾",
	[Key.favsDescription]: "來自嗶哩嗶哩的收藏夾列表",
	[Key.favsDisabled]: "B站收藏夾頁已關閉。",
	[Key.favsLoadFailed]: "載入嗶哩嗶哩收藏夾資料失敗：",
	[Key.favsEmpty]: "暫無收藏夾資料。",
	[Key.favsOtherDescription]: "查看 Bilibili 收藏夾列表",
	[Key.favsMediaCount]: "個影片",
	[Key.favsOwner]: "建立者",
	[Key.favResourcesEmpty]: "該收藏夾暫無影片。",
	[Key.favStatPlay]: "播放",
	[Key.favStatCollect]: "收藏",
	[Key.favStatDanmaku]: "彈幕",

	[Key.files]: "檔案",
	[Key.filesDescription]: "透過 API 瀏覽檔案、資料夾和目錄",
	[Key.filesSearch]: "搜尋檔案",
	[Key.filesAllTypes]: "全部類型",
	[Key.filesLoading]: "正在載入檔案...",
	[Key.filesLoadFailed]: "載入檔案失敗：",
	[Key.filesEmpty]: "暫無檔案。",
	[Key.filesRoot]: "根目錄",
	[Key.filesFolder]: "資料夾",
	[Key.filesOpen]: "開啟",
	[Key.filesDetails]: "詳細資訊",
	[Key.filesSize]: "大小",
	[Key.filesType]: "類型",
	[Key.filesModifiedAt]: "修改時間",
	[Key.filesCreatedAt]: "建立時間",
	[Key.filesUpdatedAt]: "更新時間",
	[Key.filesTotal]: "總數",
	[Key.filesPrevious]: "上一頁",
	[Key.filesNext]: "下一頁",
	[Key.filesRefresh]: "重新整理",
	[Key.filesSort]: "排序",
	[Key.filesOrderDesc]: "降序",
	[Key.filesOrderAsc]: "升序",

	[Key.friendGithubDescription]: "面向開源及私有軟體專案的託管平台",
	[Key.friendOpenAIDescription]: "AI 研究與產品平台",
	[Key.friendBilibiliDescription]: "影音社群與創作平台",
	[Key.friendXDescription]: "社群平台",
	[Key.friendDiscordDescription]: "流行的聊天和社群平台",
	[Key.friendCloudflareDescription]: "Web 效能與安全平台",

	[Key.tags]: "標籤",
	[Key.categories]: "分類",
	[Key.recentPosts]: "最新文章",

	[Key.comments]: "評論",
	[Key.commentsNotConfigured]:
		"評論功能已啟用，但尚未設定 CommentConfig.serverURL。",

	[Key.untitled]: "無標題",
	[Key.uncategorized]: "未分類",
	[Key.noTags]: "無標籤",

	[Key.wordCount]: "字",
	[Key.wordsCount]: "字",
	[Key.minuteCount]: "分鐘",
	[Key.minutesCount]: "分鐘",
	[Key.postCount]: "篇文章",
	[Key.postsCount]: "篇文章",

	[Key.themeColor]: "主題色",

	[Key.lightMode]: "亮色",
	[Key.darkMode]: "暗色",
	[Key.systemMode]: "跟隨系統",

	[Key.more]: "更多",

	[Key.author]: "作者",
	[Key.publishedAt]: "發佈於",
	[Key.license]: "許可協議",
	[Key.allRightsReserved]: "保留所有權利。",
	[Key.poweredBy]: "由",
	[Key.siteSource]: "本站開源於",
};
